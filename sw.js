...
const cacheVersion = 'v1.0.302'

console.log('service worker version', '1.0.302')
...
self.addEventListener('activate', (event) => {
  console.log('service worker active', '1.0.302', event)
  ...
  try {
    if (event.request.url.indexOf(location.origin) >= 0) {
      ...
      if (event.request.url.endsWith('.html') || matchRoute(origin_url) || event.request.url.indexOf('assets') < 0) {
        console.log('service worker', '1.0.302', event.request, event.request.url)
