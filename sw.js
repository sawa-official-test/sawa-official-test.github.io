...
const cacheVersion = 'v1.0.310'

console.log('service worker version', '1.0.310')
...
self.addEventListener('activate', (event) => {
  console.log('service worker active', '1.0.310', event)
  ...
  try {
    if (event.request.url.indexOf(location.origin) >= 0) {
      ...
      if (event.request.url.endsWith('.html') || matchRoute(origin_url) || event.request.url.indexOf('assets') < 0 || event.request.url.indexOf('sawa_logo.ico') >= 0) {
        console.log('service worker', '1.0.310')
